...
public class SuperTrend {
	...
	public SuperTrend(BarSeries series, Double multiplier, int lookbackPeriod) {
	...
	public double finalUpperBand(int index) {
		...
		if (fubPrev == -1 || upperBand < fubPrev
				|| getSeries().getBar(index - 1).getClosePrice().doubleValue() > fubPrev) {
			band = upperBand;
		} else {
			band = fubPrev;
		}
	...
	public double finalLowerBand(int index) {
		...
		if (flbPrev == -1 || lowerBand > flbPrev
				|| getSeries().getBar(index - 1).getClosePrice().doubleValue() < flbPrev) {
			band = lowerBand;
		} else {
			band = flbPrev;
		}
	...
	public String toString() {
		return this.upperBands + " " + this.lowerBands + " " + this.atri;
	}
